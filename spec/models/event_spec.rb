...
describe Event do
  ...
  describe 'methods' do
    it 'get medalists' do
      ...
      olympian_event1 = OlympianEvent.create(olympian_id: olympian1.id, event_id: event.id, medal: 'NA' )
      olympian_event2 = OlympianEvent.create(olympian_id: olympian2.id, event_id: event.id, medal: 'Silver' )
      olympian_event3 = OlympianEvent.create(olympian_id: olympian3.id, event_id: event.id, medal: 'NA' )


      expect(event.get_medalists.first).to eq(olympian2)
    end
  end
end
